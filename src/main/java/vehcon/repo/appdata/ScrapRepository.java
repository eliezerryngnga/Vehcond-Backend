...
public interface ScrapRepository extends JpaRepository<Scrap, String> {
	@Query(value =
            ...
            "    TO_DATE(SPLIT_PART(sc.letternodate, '|', 2), 'DD-MM-YYYY') AS extracted_date " +
            "  FROM " +
            "    vehiclecondemnations.scrap sc " +
            "    JOIN vehiclecondemnations.vehicles vf ON sc.applicationcode = vf.applicationcode " +
            "  WHERE " +
            "    vf.processcode = :processCode " +
            "    AND sc.letternodate LIKE '%|%' " +
            "    AND SPLIT_PART(sc.letternodate, '|', 2) ~ '^[0-9]{2}-[0-9]{2}-[0-9]{4}$' " + // Optional regex filter
            ") " +
            "SELECT " +
            ...
            nativeQuery = true)
        List<YearMonthProjection> findScrappedYearAndMonthFromLetterNoDate(@Param("processCode") Integer processCode);
}
