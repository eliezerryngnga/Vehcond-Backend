...
public interface VccTcRepository extends JpaRepository<Vcctc, String> {
    ...
    @Query(value =
            ...
            "    TO_DATE(SPLIT_PART(v.letternodate, '|', 2), 'DD-MM-YYYY') AS extracted_date " +
            "  FROM " +
            "    vehiclecondemnations.vcctc v " +
            "    JOIN vehiclecondemnations.vehicles vf ON v.applicationcode = vf.applicationcode " +
            "  WHERE " +
            "    vf.processcode = :processCode " +
            "    AND v.letternodate LIKE '%|%' " +
            "    AND SPLIT_PART(v.letternodate, '|', 2) ~ '^[0-9]{2}-[0-9]{2}-[0-9]{4}$' " + // Optional regex filter
            ") " +
            "SELECT " +
            ...
            nativeQuery = true)
        List<YearMonthProjection> findVcctcYearAndMonthFromLetterNoDate(@Param("processCode") Integer processCode);
}